...
application.config['UPLOAD_FOLDER'] = param['upload_location']
application.config["MONGO_URI"] = param['server_location']
application.config["sampleCSV"] = param['sampleCSV_location']
...
@application.route("/uploadPredict", methods=['POST'])
@cross_origin()
def predict_file():
    try:
        if 'email' in session:
...
@application.route("/downloadsample", methods=['POST'])
@cross_origin()
def download_sample():
    try:
        if 'email' in session:
            ...
            if request.method == 'POST':
                samplepath=application.config["sampleCSV"]

                return send_file(samplepath, as_attachment=True)
